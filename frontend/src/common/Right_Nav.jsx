...
const Right_Nav = () => {
  ...
  const [messages, setMessages] = useState([
    {
      role: "bot",
      text: "Thank you for contacting Kryzen. An award-winning Web, Mobile App and Product Development Company.",
    },
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    const formData = {
      ...
      formType: "Right Nav Contact Form",
    };

    try {
      const response = await postApi("/api/send-email", formData);
      setMessage(response.data?.message || "Message sent successfully!");
      e.target.reset();
    } catch (error) {
      setMessage(getApiErrorMessage(error, "Failed to send message. Please try again."));
    } finally {
      setLoading(false);
    }
  };

  const sendMsgWhatsapp = (number, text) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const extractOutput = (payload) => {
        ...
        if (Array.isArray(payload)) return extractOutput(payload[0]);
      ...
      setMessages((prev) => {
        const withoutTyping = prev.filter((item) => item.typing !== typingToken);
        ...
        <div className="relative">
          <div className="mb-3" onClick={() => setOpen(true)}>
            <div className="bg-[#005D89] p-3 rounded-lg shadow-lg transition-all duration-100 hover:scale-110 cursor-pointer">
              <svg className="w-6 h-6 text-white animate-surprise" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
          ...
          {open && (
            <div className="fixed top-27 right-0 z-50 bg-[#005D89] text-white w-70 p-6 rounded-lg shadow-xl transition-all duration-300 animate-slide-in">
              <button onClick={() => setOpen(false)} className="absolute top-4 left-4 text-2xl font-bold text-white cursor-pointer">
                x
              </button>
              <div className="mt-6">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-2 w-full max-w-md mx-auto">
                  ...
                  <div className="flex w-full">
                    <select name="countryCode" className="rounded-l px-2 py-1 text-black bg-gray-200">
          ...
          <div
            ...
            onClick={() =>
              sendMsgWhatsapp("+917041252829", "Hello, I am interested in your sales services. Please provide more information. Thanks!")
            }
            onMouseEnter={() => setisHover(true)}
            onMouseLeave={() => setisHover(false)}
          >
            <div className="bg-[#005D89] p-3 rounded-lg shadow-lg transition-all duration-300 hover:scale-110 cursor-pointer">
              <svg className="w-6 h-6 text-white animate-surprise-delayed" fill="currentColor" viewBox="0 0 24 24">
      ...
      {chatOpen && (
        <div className="fixed bottom-4 right-4 z-50 w-80 bg-white rounded-xl shadow-2xl overflow-hidden border">
          <div className="bg-[#005D89] text-white flex items-center justify-between px-4 py-3">
            ...
            <button className="text-white text-xl leading-none" onClick={() => setChatOpen(false)}>
              x
            </button>
          </div>
          <div className="p-3 h-64 overflow-y-auto bg-gray-50">
            {messages.map((item, index) => (
              <div key={index} className={`mb-2 flex ${item.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`${item.role === "user" ? "bg-[#005D89] text-white" : "bg-white text-gray-800"} px-3 py-2 rounded-lg max-w-auto shadow`}>
                  {item.text}
                </div>
              </div>
            ))}
          ...
          <form className="p-3 border-t flex items-center gap-2" onSubmit={handleSendMessage}>
            ...
            />
            <button type="submit" className="bg-[#005D89] text-white px-3 py-2 rounded">
              Send
            </button>
          </form>
        </div>
