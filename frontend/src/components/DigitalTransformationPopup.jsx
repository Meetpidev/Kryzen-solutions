...
export default function DigitalTransformationPopup() {
  ...
  const impressions = useMemo(() => readNumberFromStorage(STORAGE_KEYS.impressions, 0), []);
  const lastShownAt = useMemo(() => readNumberFromStorage(STORAGE_KEYS.lastShownAt, 0), []);

  useEffect(() => {
    if (impressions >= 2) return undefined;
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await postApi("/api/send-email", {
        ...
        formType: "Digital Transformation Popup",
      });

      setStatusMsg(response.data?.message || "Message sent successfully!");
      setFormData({ name: "", email: "", phone: "", message: "", countryCode: "+91" });
      setTimeout(() => close(), 1500);
    } catch (error) {
      console.error("Email send error:", error);
      setStatusMsg(getApiErrorMessage(error, "Failed to send message. Please try again."));
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="relative w-full max-w-6xl max-h-[90vh] overflow-y-auto rounded-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 bg-white rounded-xl overflow-hidden shadow-2xl">
          <div className="bg-[#eaf3ff] p-4 sm:p-6 md:p-8">
            <h2 className="text-xl sm:text-2xl md:text-[28px] font-extrabold text-[#1a273b]">
              Let's Navigate Digital Transformation Together!!!
            </h2>
            <hr className="w-12 border-[#1a273b] my-2 sm:my-3" />
            <p className="text-[#1a273b] text-sm sm:text-base">Like a Caterpillar turning into a Butterfly.</p>
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-3">
              <div>
                <input
                  ...
                  className={`w-full rounded border p-3 text-sm sm:text-base focus:outline-none ${
                    touched.name && formErrors.name ? "border-red-500" : "border-gray-300"
                  }`}
                  required
                />
                {touched.name && formErrors.name && <p className="text-red-600 text-xs mt-1">{formErrors.name}</p>}
              </div>
              ...
              <div>
                <input
                  ...
                  required
                />
                {touched.email && formErrors.email && <p className="text-red-600 text-xs mt-1">{formErrors.email}</p>}
              </div>
              ...
              <div>
                <div className="flex gap-2">
                  <select
                    ...
                    className="w-[70px] sm:w-[90px] rounded border p-3 focus:outline-none border-gray-300 text-sm sm:text-base"
                  >
                  ...
                  <input
                    ...
                    className={`flex-1 rounded border p-3 text-sm sm:text-base focus:outline-none ${
                      touched.phone && formErrors.phone ? "border-red-500" : "border-gray-300"
                    }`}
                    required
                  />
                </div>
                {touched.phone && formErrors.phone && <p className="text-red-600 text-xs mt-1">{formErrors.phone}</p>}
              </div>
              ...
              <div className="mb-2">
                <textarea
                  ...
                  className={`w-full resize-none rounded border p-3 text-sm sm:text-base focus:outline-none ${
                    touched.message && formErrors.message ? "border-red-500" : "border-gray-300"
                  }`}
                  required
                />
                {touched.message && formErrors.message && <p className="text-red-600 text-xs mt-1">{formErrors.message}</p>}
              </div>
              ...
              {statusMsg && <p className="mt-2 text-center text-sm font-semibold text-[#1a273b]">{statusMsg}</p>}
            </form>
          </div>
        </div>
      </div>
